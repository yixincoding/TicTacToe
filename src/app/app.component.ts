...
export class AppComponent {
  ...
  async newMove(index: number, event: any) {
    ...
    if (this.mode === 0) {
      while (this.dataService.getActiveTicTac() === -2) {
        const random: number = this.getRandomInt(0, 8);
        if (this.array[random].isTaken === 0) {
          this.dataService.setActiveTicTac(random);
        }
      }

      const x: number = this.dataService.getActiveTicTac();
      let y = 0;
      for (y = 0; y < 9; y++) {
      ...
      if (y === 9) { // cannot taken
        while (1) {
          const random: number = this.getRandomInt(0, 8);
          if (this.array[x].placeArray[random].index === 0) {
            await this.sleep(1500);
            this.array[x].placeArray[random].index = this.dataService.getTurn();
            this.array[x].placeArray[random].icon = this.iconArr[this.dataService.getTurn()];
            this.dataService.nextTurn();
            if (this.array[random].isTaken === 0) {
              this.dataService.setActiveTicTac(random);
            } else {
  ...
  checkAITaken(index: number, put: boolean): boolean {
    ...
    const _tictac: number = this.dataService.getActiveTicTac();
    if (this.array[_tictac].placeArray[x * 3 + (y + 1) % 3].index === _turn
      && this.array[_tictac].placeArray[x * 3 + (y + 2) % 3].index === _turn) {
      this.array[_tictac].isTaken = put ? _turn : 0; // row
      return true;
    } else if (this.array[_tictac].placeArray[y + (x + 1) % 3 * 3].index === _turn
      && this.array[_tictac].placeArray[y + (x + 2) % 3 * 3].index === _turn) {
      this.array[_tictac].isTaken = put ? _turn : 0; // column
      return true;
    } else if (x === y && (this.array[_tictac].placeArray[(x + 1) % 3 * 3 + (y + 1) % 3].index === _turn
      && this.array[_tictac].placeArray[(x + 2) % 3 * 3 + (y + 2) % 3].index === _turn)) {
      this.array[_tictac].isTaken = put ? _turn : 0; // left dig
      return true;
    } else if (x + y === 2 && (this.array[_tictac].placeArray[(x + 2) % 3 * 3 + (y + 1) % 3].index === _turn
      && this.array[_tictac].placeArray[(x + 1) % 3 * 3 + (y + 2) % 3].index === _turn)) {
  ...
  checkWinner(index: number) {
    const _turn: number = this.dataService.getLastTurn();
    ...
    } else if (x === y && (this.array[(x + 1) % 3 * 3 + (y + 1) % 3].isTaken === _turn
      && this.array[(x + 2) % 3 * 3 + (y + 2) % 3].isTaken === _turn)) {
      this.winner = _turn; // left dig
    } else if (x + y === 2 && (this.array[(x + 2) % 3 * 3 + (y + 1) % 3].isTaken === _turn
      && this.array[(x + 1) % 3 * 3 + (y + 2) % 3].isTaken === _turn)) {
      this.winner = _turn;
    } else {
      for (let i = 0; i < 9; i++) {
